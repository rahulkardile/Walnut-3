...
const Edit = () => {
  ...
  const GetData = async () => {
    const data = await fetch(`/api/blog/get/${id}`);
    const { data: { title, summary, content, MainPoster } } = await data.json();
  ...
  return (
    <>
      ...
      <section className="flex flex-col gap-7 w-screen items-center mt-6">
        <h1 className="text-2xl portrait:text-xl font-semibold">Edit Blog</h1>
        <form
